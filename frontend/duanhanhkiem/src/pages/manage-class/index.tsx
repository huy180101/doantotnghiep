...
import { fetcher_$GET, fetcher_$POST, notificationError, notificationSuccess } from "@/common/functionglobal";
...
export default function ManageClass(props: any) {
    ...
    },
    {
      ...
      render: (_, record, index) => (
        <Space size="middle">
          ...
          <Tooltip placement="topLeft" title='Chọn giáo viên vào lớp'>
            <CheckCircleOutlined onClick={() => handleOpenSelecTeacherModal(record, index)} />
          </Tooltip>
          <Tooltip placement="topLeft" title='Danh sách sinh viên'>
  ...
  }, [error, listRes, listTeacher, errorTeacher]);
        ...
        const valueTrigger = json.map((obj: any) => {
          ...
          if (!!obj['Ngày Sinh']) {
            let arrayBirthday = obj['Ngày Sinh'].toString().split("/"),
              birthdayToISOString = dayjs(`${arrayBirthday[2]}-${arrayBirthday[1]}-${arrayBirthday[0]}`).toISOString();
            data.birthday = dayjs(`${birthdayToISOString}`).utc().hour(0).minute(0).second(0).millisecond(0);
          }
          else {
            data.birthday = "";
          }
